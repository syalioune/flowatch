...
**And** the legacy `.conn-pill[data-state="mock"]` value is removed from `styles.css` and any component that branched on it — the four-state contract (`ok`, `err`, `pending`, `unset`) is the complete enumeration (per CLAUDE.md "the app talks only to the live engine — there is no mock fallback").
