...
while IFS=$'\t' read -r epic_num epic_title story_num story_title body_enc milestone; do
  ...
  # Don't clobber existing curated files. If a file with the same prefix
  # already exists, append a "-vN" suffix.
  if [ -f "$filepath" ]; then
    n=2
    while [ -f "$OUT_DIR/${prefix}_${safe_title}-v${n}.md" ]; do n=$((n+1)); done
    filename="${prefix}_${safe_title}-v${n}.md"
    filepath="$OUT_DIR/$filename"
  fi
