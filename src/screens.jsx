...
import { Icon, PageHead, fmtTime, fmtDue, toast } from './components.jsx';
...
const ErrorBox = ({ error, onRetry }) => (
  <div className="empty" style={{padding:24, color:"var(--bad)"}}>
    ...
    {onRetry && <button className="btn" data-size="sm" onClick={onRetry}>Retry</button>}
  </div>
);
const EmptyRow = ({ cols, msg = "No records." }) => (
  <tr><td colSpan={cols} className="empty" style={{padding:24}}>{msg}</td></tr>
);
...
export const Dashboard = ({ onOpenInspector, onNav }) => {
  ...
  const instances = useApi(() => api.listProcessInstances({ size: 8, sort: "startTime", order: "desc" }), []);
  const tasks = useApi(() => api.listTasks({ size: 0 }), []);
  const failingJobs = useApi(() => api.listJobs({ withException: true, size: 5 }), []);
  const deadLetter = useApi(() => api.listDeadLetterJobs({ size: 0 }).catch(() => ({ total: 0 })), []);
  ...
  return (
    <div className="page">
      ...
      <div style={{display:"grid", gridTemplateColumns:"1.4fr 1fr", gap:18}}>
        <div className="panel">
          <div className="panel-hd">
            ...
            <button className="btn" data-size="sm" data-variant="ghost" onClick={() => onNav("instances")}>View all</button>
          </div>
          <div style={{overflow:"auto"}}>
            <table className="tbl" style={{border:0, borderRadius:0}}>
              <thead><tr><th>Business key</th><th>Definition</th><th>Activity</th><th>Started</th><th></th></tr></thead>
              <tbody>
                {instances.loading && <EmptyRow cols={5} msg="Loading…" />}
                {instances.error && <EmptyRow cols={5} msg={String(instances.error.message || instances.error)} />}
                {!instances.loading && !instances.error && (instances.data?.data || []).length === 0 && (
        ...
        <div className="panel">
          ...
          <div className="panel-body" style={{padding:0}}>
            {failingJobs.loading && <div className="empty" style={{padding:20}}>Loading…</div>}
            {failingJobs.error && <ErrorBox error={failingJobs.error} />}
            {failingJobs.data && (failingJobs.data.data || []).map((j) => (
              <div key={j.id} style={{padding:"12px 14px", borderBottom:"1px solid var(--line)"}}>
                <div style={{display:"flex", alignItems:"center", gap:8}}>
                  <span className="badge" data-tone="bad"><span className="dot"/>{j.jobType || "job"}</span>
                  <span className="mono" style={{fontSize:11.5, color:"var(--fg-mute)"}}>{j.id}</span>
                ...
                <div className="mono" style={{fontSize:10.5, marginTop:4, color:"var(--fg-mute)"}}>
                  {j.elementId ? `${j.elementId} · ` : ""}pi:{j.processInstanceId || "—"}
                </div>
                <div style={{display:"flex", gap:6, marginTop:8}}>
                  <button className="btn" data-size="sm" onClick={() => api.executeJob(j.id).then(failingJobs.reload)}>
      ...
      <div className="panel" style={{marginTop:18}}>
        <div className="panel-hd"><span className="panel-title">Process definitions</span>
          <button className="btn" data-size="sm" data-variant="ghost"
                  style={{marginLeft:"auto"}} onClick={() => onNav("definitions")}>View all</button>
        </div>
        <div className="panel-body" style={{padding:0}}>
          {definitions.loading && <div className="empty" style={{padding:20}}>Loading…</div>}
          {definitions.error && <ErrorBox error={definitions.error} />}
          {topDefs.map((d) => (
            <div key={d.id} style={{display:"flex", alignItems:"center", padding:"10px 14px",
                                     borderBottom:"1px solid var(--line)", gap:10}}>
              <div style={{flex:1, minWidth:0}}>
                <div style={{fontSize:13, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{d.name || d.key}</div>
                <div className="mono" style={{fontSize:11, color:"var(--fg-mute)"}}>v{d.version} · {d.key}</div>
              </div>
              <span className="badge" data-tone={d.suspended ? "warn" : "ok"}>
                <span className="dot"/>{d.suspended ? "suspended" : "active"}
              </span>
            </div>
          ))}
...
export const Deployments = ({ onOpenInspector }) => {
  ...
  const deployments = useApi(() => api.listDeployments({ size: 200, sort: "deployTime", order: "desc" }), []);
  const rows = (deployments.data?.data || []).filter((d) =>
    !filter ||
  ...
  return (
    <div className="page">
      <PageHead title="Deployments"
                ...
                onOpenInspector={onOpenInspector}
                actions={<>
                  <button className="btn" onClick={deployments.reload}><Icon name="refresh" size={13}/>Refresh</button>
                </>} />
      <div className="tbl-wrap">
        <div className="tbl-toolbar">
          <div className="search-mini"><Icon name="search" size={12}/>
            <input placeholder="Filter by name, category, tenant…"
        ...
        <table className="tbl">
          <thead><tr>
            <th>Name</th><th>ID</th><th>Category</th><th>Tenant</th>
            <th>Deployed</th><th></th>
          </tr></thead>
          <tbody>
            {deployments.loading && <EmptyRow cols={6} msg="Loading…" />}
            {deployments.error && <EmptyRow cols={6} msg={String(deployments.error.message || deployments.error)} />}
            ...
            {rows.map((d) => (
              <tr key={d.id}>
                ...
                <td>{d.category ? <span className="badge" data-tone="neutral">{d.category}</span> : <span className="mute">—</span>}</td>
                <td className="mono">{d.tenantId || <span className="mute">—</span>}</td>
                <td className="mute mono">{fmtTime(d.deploymentTime)}</td>
                <td>
                  <button className="btn" data-size="sm" data-variant="ghost"
                          onClick={() => remove(d.id)}
                          title="Delete (cascade)">×</button>
                </td>
              </tr>
            ))}
          </tbody>
...
export const ProcessDefinitions = ({ onOpenInspector, onNav }) => {
  ...
  return (
    <div className="page">
      <PageHead title="Process definitions"
      ...
      <div className="tbl-wrap">
        <div className="tbl-toolbar">
          ...
          <button className="btn" data-size="sm" data-variant={showSuspended ? "primary" : "ghost"}
        ...
        <table className="tbl">
          <thead><tr>
            <th>Definition</th><th>Key</th><th>Version</th><th>Status</th>
            <th>Tenant</th><th>Category</th><th></th>
          </tr></thead>
          <tbody>
            {definitions.loading && <EmptyRow cols={7} msg="Loading…" />}
            {definitions.error && <EmptyRow cols={7} msg={String(definitions.error.message || definitions.error)} />}
              ...
              <tr key={d.id}>
                ...
                <td>
                  <div style={{display:"flex", gap:6}}>
                    ...
                    <button className="btn" data-size="sm" data-variant="ghost" onClick={() => toggle(d)}>
      ...
      {startDialog && (
        <StartProcessDialog definition={startDialog}
    ...
    if (named.length) {
      try {
        payload.variables = named.map((v) => {
          const c = coerce(v);
          if (v.type === "integer" && Number.isNaN(c.value)) throw new Error(`'${v.name}' is not an integer`);
          if (v.type === "double"  && Number.isNaN(c.value)) throw new Error(`'${v.name}' is not a number`);
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{width: 620}}>
        ...
        <div className="modal-bd">
          <div className="form-row">
            <label>Business key <span className="mono">optional</span></label>
            <input className="input" value={businessKey}
            ...
            <div style={{display:"flex", flexDirection:"column", gap:6}}>
              {vars.map((v, i) => (
                <div key={i} style={{display:"grid",
                                     gridTemplateColumns:"1fr 110px 1.4fr 28px", gap:6}}>
                  <input className="input mono" placeholder="name" value={v.name}
                         onChange={(e) => updateRow(i, { name: e.target.value })} />
                  <select className="select" value={v.type}
                          onChange={(e) => updateRow(i, { type: e.target.value })}>
                    {VAR_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                  </select>
                  {v.type === "boolean" ? (
                    <select className="select" value={v.value || "false"}
              ...
              <button className="btn" data-size="sm" data-variant="ghost"
                      onClick={addRow} style={{alignSelf:"flex-start"}}>
                <Icon name="plus" size={11}/>Add variable
              </button>
            </div>
          </div>
          {parseError && (
            <div className="badge" data-tone="bad" style={{marginTop:4}}>{parseError}</div>
          )}
        </div>
...
export const ProcessInstances = ({ onOpenInspector }) => {
  const instances = useApi(() => api.listProcessInstances({ size: 200, sort: "startTime", order: "desc" }), []);
  ...
  const vars = useApi(() => sel ? api.getProcessInstanceVariables(sel.id) : Promise.resolve({ data: [] }), [sel?.id]);
  ...
  return (
    <div className="page" style={{paddingBottom:24}}>
      <PageHead title="Process instances"
                subtitle="Currently-running instances across all definitions."
                endpoints={DATA.endpoints.instances}
                onOpenInspector={onOpenInspector}
                actions={<button className="btn" onClick={instances.reload}><Icon name="refresh" size={13}/>Refresh</button>}/>
      <div style={{display:"grid", gridTemplateColumns:"minmax(0,1fr) 400px", gap:16}}>
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr>
              <th>Business key</th><th>Definition</th><th>Activity</th>
              <th>Started by</th><th>Started</th><th>State</th>
            </tr></thead>
            <tbody>
              {instances.loading && <EmptyRow cols={6} msg="Loading…" />}
              {instances.error && <EmptyRow cols={6} msg={String(instances.error.message || instances.error)} />}
              {!instances.loading && !instances.error && list.length === 0 && (
                <EmptyRow cols={6} msg="No running process instances." />
              )}
              {list.map((p) => (
                <tr key={p.id} data-selected={p.id === selectedId ? "1" : "0"}
        ...
        <div className="panel">
          {!sel && <div className="empty" style={{padding:24}}>Select an instance.</div>}
          {sel && (
            <>
              <div className="panel-hd">
                <span className="panel-title">{sel.businessKey || sel.id}</span>
                <span className="mono mute" style={{marginLeft:"auto", fontSize:11}}>{sel.id}</span>
              </div>
              <div className="panel-body">
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14}}>
                  <Info label="Definition" value={sel.processDefinitionName || sel.processDefinitionKey}/>
                ...
                <div className="drawer-sect">Variables
                  <span className="mono mute" style={{textTransform:"none", fontSize:10, letterSpacing:0, marginLeft:6}}>
                    GET /runtime/process-instances/{sel.id}/variables
                  </span>
                </div>
                {vars.loading && <div className="empty" style={{padding:14}}>Loading…</div>}
                {vars.error && <ErrorBox error={vars.error} />}
                {vars.data && (
                  <div style={{display:"grid", gridTemplateColumns:"1fr", gap:6}}>
                    {(vars.data.data || vars.data || []).length === 0 && <div className="mute">No variables.</div>}
                    {(vars.data.data || vars.data || []).map((v) => (
                      <div key={v.name} style={{display:"flex", justifyContent:"space-between",
                                                 padding:"6px 10px", background:"var(--bg-sunken)",
                                                 border:"1px solid var(--line)", borderRadius:6}}>
                        <span className="mono" style={{fontSize:11.5, color:"var(--fg-soft)"}}>{v.name}</span>
                ...
                <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                  <button className="btn" data-size="sm" data-variant="danger" onClick={() => cancel(sel.id)}>
...
const Info = ({ label, value, mono }) => (
  <div>
    <div className="text-xs mute" style={{textTransform:"uppercase", letterSpacing:"0.06em"}}>{label}</div>
    <div className={mono ? "mono mt-2" : "mt-2"} style={{fontSize:13}}>{value}</div>
  </div>
);
  ...
  return (
    <div className="page">
      <PageHead title="Jobs"
                ...
                onOpenInspector={onOpenInspector}
                actions={<button className="btn" onClick={jobs.reload}><Icon name="refresh" size={13}/>Refresh</button>}/>
      <div className="tbl-wrap">
        <div className="tbl-toolbar">
          <div className="seg-row">
            <button className="seg-btn" data-on={tab==="jobs"?"1":"0"} onClick={() => setTab("jobs")}>Jobs</button>
            <button className="seg-btn" data-on={tab==="timers"?"1":"0"} onClick={() => setTab("timers")}>Timers</button>
            <button className="seg-btn" data-on={tab==="deadletter"?"1":"0"} onClick={() => setTab("deadletter")}>Dead-letter</button>
          </div>
          <span className="mute mono text-xs" style={{marginLeft:"auto"}}>
        ...
        <table className="tbl">
          <thead><tr>
            <th>Job</th><th>Element</th><th>Process</th>
            <th>Due</th><th>Retries</th><th>Exception</th><th></th>
          </tr></thead>
          <tbody>
            ...
            {!jobs.loading && !jobs.error && list.length === 0 && (
              <EmptyRow cols={7} msg={tab === "deadletter" ? "Dead-letter queue is empty." : "No jobs."} />
            )}
            {list.map((j) => (
              <tr key={j.id}>
                ...
                <td className="mono" style={{color: j.retries === 0 ? "var(--bad)" : "var(--fg)"}}>{j.retries}</td>
                <td className="mono" style={{maxWidth:240, overflow:"hidden", textOverflow:"ellipsis", color: j.exceptionMessage ? "var(--bad)" : "var(--fg-mute)"}}>
                  {j.exceptionMessage || "—"}
                </td>
                <td>
                  <button className="btn" data-size="sm" onClick={() => retry(j)} title={tab === "deadletter" ? "Move back" : "Execute now"}>
...
export const Tasks = ({ onOpenInspector }) => {
  ...
  const fetcher = () => {
    ...
    if (filter === "mine" && cfg.username) return api.listTasks({ assignee: cfg.username, size: 200 });
  ...
  const taskVars = useApi(
    () => sel ? api.getTaskVariables(sel.id) : Promise.resolve({ data: [] }),
    [sel?.id]
  );
  ...
  return (
    <div className="page" style={{padding:"24px 28px"}}>
      <PageHead title="Tasks"
                ...
                onOpenInspector={onOpenInspector}
                actions={<button className="btn" onClick={tasks.reload}><Icon name="refresh" size={13}/>Refresh</button>}/>
      <div className="split-tasks">
        <div className="panel">
          <div className="panel-hd" style={{flexDirection:"column", alignItems:"stretch", padding:0}}>
            <div className="tbl-toolbar" style={{margin:0, borderBottom:0, padding:"10px 12px"}}>
              <div className="seg-row">
                <button className="seg-btn" data-on={filter==="mine"?"1":"0"} onClick={() => setFilter("mine")}>Assigned to me</button>
                <button className="seg-btn" data-on={filter==="unassigned"?"1":"0"} onClick={() => setFilter("unassigned")}>Unassigned</button>
                <button className="seg-btn" data-on={filter==="all"?"1":"0"} onClick={() => setFilter("all")}>All</button>
              </div>
            </div>
          </div>
          <div style={{flex:1, overflowY:"auto"}}>
            {tasks.loading && <div className="empty" style={{padding:20}}>Loading…</div>}
            {tasks.error && <ErrorBox error={tasks.error} />}
            {!tasks.loading && !tasks.error && list.length === 0 && (
              <div className="empty" style={{padding:20}}>No tasks for this filter.</div>
            )}
            {list.map((t) => (
              <div key={t.id}
                ...
                <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:4}}>
                  <div style={{fontSize:13, fontWeight:500, flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{t.name || t.id}</div>
                  {t.dueDate && (
                    <span className="badge" data-tone="neutral" style={{fontSize:10}}>{fmtDue(t.dueDate)}</span>
                  )}
                </div>
                <div style={{fontSize:11.5, color:"var(--fg-mute)", display:"flex", gap:8}}>
                  <span>{t.processDefinitionName || t.processDefinitionKey || "—"}</span>
                  <span>·</span>
                  <span className="mono">{t.assignee || (t.candidateGroup ? `group:${t.candidateGroup}` : "unclaimed")}</span>
                </div>
                <div style={{fontSize:10.5, color:"var(--fg-mute)", marginTop:3}} className="mono">
        ...
        <div className="panel">
          {!sel && <div className="empty" style={{padding:24}}>Select a task.</div>}
          {sel && (
            <>
              <div className="panel-hd">
              ...
              <div className="panel-body">
                <div style={{display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:10, marginBottom:18}}>
                  <Info label="Process" value={sel.processDefinitionName || sel.processDefinitionKey || "—"}/>
                ...
                <div className="drawer-sect">Form
                  <span className="mono mute" style={{textTransform:"none", fontSize:10, letterSpacing:0, marginLeft:6}}>
                    GET /form/form-data?taskId={sel.id}
                  </span>
                </div>
                {form.loading && <div className="empty" style={{padding:14}}>Loading…</div>}
                {!form.loading && !form.data && <div className="mute" style={{padding:"8px 0"}}>No form attached to this task.</div>}
                {form.data && (
                  <div style={{maxWidth:560}}>
                    ...
                    {(form.data.formProperties || []).map((f) => (
                      <div className="form-row" key={f.id}>
                        <label>{f.name || f.id} {f.required && <span className="req">*</span>}
                          <span className="mono">{f.type}</span>
                        </label>
                        {f.type === "enum" && Array.isArray(f.enumValues) && (
                          <div className="seg-row">
                            {f.enumValues.map((v) => (
                              <button key={v.id || v} className="seg-btn">{v.name || v.id || v}</button>
                            ))}
                          </div>
                        )}
                        {f.type !== "enum" && (
                ...
                <div style={{display:"flex", gap:8, marginTop:18, paddingTop:14, borderTop:"1px solid var(--line)"}}>
                  {!sel.assignee && (
                    <button className="btn" data-variant="primary" onClick={claim}>Claim</button>
                  )}
                  <button className="btn" data-variant={sel.assignee ? "primary" : "ghost"} onClick={complete}>
                    Complete
                  </button>
                </div>

                <div className="drawer-sect">Variables in scope</div>
                {taskVars.loading && <div className="empty" style={{padding:14}}>Loading…</div>}
                {taskVars.data && (
                  <div style={{display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:6}}>
                    {((taskVars.data.data || taskVars.data) || []).length === 0 && <div className="mute">No variables.</div>}
                    {((taskVars.data.data || taskVars.data) || []).map((v) => (
                      <div key={v.name} style={{display:"flex", justifyContent:"space-between",
                                                 padding:"6px 10px", background:"var(--bg-sunken)",
                                                 border:"1px solid var(--line)", borderRadius:6}}>
                        <span className="mono" style={{fontSize:11.5, color:"var(--fg-soft)"}}>{v.name}</span>
...
export const History = ({ onOpenInspector }) => {
  const completed = useApi(() => api.listHistoricInstances({ finished: true, size: 100, sort: "endTime", order: "desc" }), []);
  ...
  const audit = useApi(
    () => sel ? api.listHistoricActivities({ processInstanceId: sel.id, size: 200, sort: "startTime" }) : Promise.resolve({ data: [] }),
    [sel?.id]
  );
  const vars = useApi(
    () => sel ? api.listHistoricVariables({ processInstanceId: sel.id, size: 200 }) : Promise.resolve({ data: [] }),
    [sel?.id]
  );
  ...
  return (
    <div className="page" style={{padding:"24px 28px"}}>
      <PageHead title="History"
                ...
                onOpenInspector={onOpenInspector}
                actions={<button className="btn" onClick={completed.reload}><Icon name="refresh" size={13}/>Refresh</button>}/>
      <div style={{display:"grid", gridTemplateColumns:"minmax(0,1fr) 480px", gap:16}}>
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr>
              <th>Business key</th><th>Definition</th><th>Duration</th>
              <th>Started</th><th>Ended</th>
            </tr></thead>
            <tbody>
              {completed.loading && <EmptyRow cols={5} msg="Loading…" />}
              {completed.error && <EmptyRow cols={5} msg={String(completed.error.message || completed.error)} />}
              {!completed.loading && !completed.error && list.length === 0 && (
                <EmptyRow cols={5} msg="No completed instances yet." />
              )}
              {list.map((p) => (
                <tr key={p.id} data-selected={p.id===selectedId?"1":"0"} onClick={() => setSelectedId(p.id)}>
                  <td className="mono">{p.businessKey || p.id}</td>
                  <td>{p.processDefinitionName || p.processDefinitionKey}</td>
          ...
          {sel && (
            <>
              ...
              <div className="panel-body" style={{paddingTop:0}}>
                <div className="tabs">
                  <div className="tab" data-active={tab==="audit"?"1":"0"} onClick={() => setTab("audit")}>Audit trail</div>
                  <div className="tab" data-active={tab==="variables"?"1":"0"} onClick={() => setTab("variables")}>Variables</div>
                </div>
                {tab === "audit" && (
                  <>
                    {audit.loading && <div className="empty" style={{padding:14}}>Loading…</div>}
                    ...
                    {audit.data && (
                      <div style={{position:"relative", paddingLeft:18}}>
                        ...
                        {(audit.data.data || []).length === 0 && <div className="mute">No activity records.</div>}
                        {(audit.data.data || []).map((a) => (
                          <div key={a.id} style={{position:"relative", paddingBottom:14}}>
                            <div style={{position:"absolute", left:-15, top:4, width:11, height:11, borderRadius:"50%",
                                         background: a.activityType === "endEvent" ? "var(--fg)" :
                                                     a.activityType === "userTask" ? "var(--accent)" : "var(--bg-elev)",
                                         border:`1.5px solid ${a.activityType === "userTask" ? "var(--accent)" : "var(--fg-soft)"}`}}/>
                            <div style={{fontSize:13, fontWeight:500}}>{a.activityName || a.activityId}</div>
                            <div className="mono" style={{fontSize:11, color:"var(--fg-mute)"}}>
                              {a.activityType} · {a.activityId}
                              {a.assignee && <> · assignee: {a.assignee}</>}
                            </div>
                            <div className="mono" style={{fontSize:11, color:"var(--fg-soft)", marginTop:2}}>
                  ...
                  <>
                    ...
                    {vars.data && (
                      <table className="tbl" style={{border:0}}>
                        ...
                        <tbody>
                          {(vars.data.data || []).length === 0 && <tr><td colSpan={3} className="mute" style={{padding:14}}>No variables.</td></tr>}
                          {(vars.data.data || []).map((v) => (
                            <tr key={v.id}><td className="mono">{v.variableName}</td>
                              <td className="mono">{typeof v.value === "string" ? `"${v.value}"` : String(v.value)}</td>
...
export const Identity = ({ onOpenInspector }) => {
  ...
  return (
    <div className="page">
      <PageHead title="Users & groups"
                subtitle="Identity records used by candidate-user, candidate-group, and assignee bindings."
                endpoints={DATA.endpoints.identity}
                onOpenInspector={onOpenInspector}
                actions={<button className="btn" onClick={() => { users.reload(); groups.reload(); }}>
                  <Icon name="refresh" size={13}/>Refresh
                </button>}/>
      <div className="tabs">
        <div className="tab" data-active={tab==="users"?"1":"0"} onClick={() => setTab("users")}>Users · {users.data?.total ?? 0}</div>
        <div className="tab" data-active={tab==="groups"?"1":"0"} onClick={() => setTab("groups")}>Groups · {groups.data?.total ?? 0}</div>
      </div>
      {tab === "users" && (
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>User</th><th>ID</th><th>Email</th><th>Display name</th></tr></thead>
            <tbody>
              {users.loading && <EmptyRow cols={4} msg="Loading…" />}
              {users.error && <EmptyRow cols={4} msg={String(users.error.message || users.error)} />}
                  ...
                  <tr key={u.id}>
                    <td>
                      <div style={{display:"flex", alignItems:"center", gap:10}}>
                        <div className="avatar" style={{width:28, height:28, fontSize:10, background:"var(--bg-sunken)", color:"var(--fg)", border:"1px solid var(--line)"}}>
                          {initials}
                        </div>
      ...
      {tab === "groups" && (
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>Group</th><th>ID</th><th>Type</th></tr></thead>
            <tbody>
              {groups.loading && <EmptyRow cols={3} msg="Loading…" />}
              {groups.error && <EmptyRow cols={3} msg={String(groups.error.message || groups.error)} />}
              {!groups.loading && !groups.error && groupList.length === 0 && (
                <EmptyRow cols={3} msg="No groups." />
              )}
              {groupList.map((g) => (
                <tr key={g.id}>
                  <td><b style={{fontWeight:500}}>{g.name || g.id}</b></td>
                  <td className="mono mute">{g.id}</td>
                  <td>{g.type ? <span className="badge" data-tone={g.type==="security"?"warn":"neutral"}>{g.type}</span> : <span className="mute">—</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="page">
      ...
      {(tenants || []).length === 0 && (
        <div className="empty" style={{padding:30}}>
          No tenant-scoped resources found. Deploy a process with a <span className="mono">tenantId</span> to populate this view.
        </div>
      )}
        ...
        {(tenants || []).map((t) => (
          <div key={t.id} className="kpi" style={{padding:18}}>
            <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:10}}>
              ...
              <span className="mono mute" style={{marginLeft:"auto", fontSize:11}}>{t.id}</span>
            </div>
