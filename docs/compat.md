...
| App | `/flowable-rest/app-api` | 🟡 partially mounted — `app-repository/app-definitions` works (read-only verified empty); `app-runtime/app-instances` returns "No endpoint" | FR-55 partial |
...
| **FR-53** Batches | `/management/batches` | ✅ | Endpoint returns 200 with paginated data (empty in this engine state). Per Flowable docs: `/management/batches/{id}` and `/management/batches/{id}/batch-parts` are available too. |
| **FR-54** Event subscriptions | `/runtime/event-subscriptions` | ✅ | Endpoint returns 200. List works; filtering by `processInstanceId`, `eventType`, `eventName`, `tenantId` per Flowable REST spec. |
| **FR-55** App definitions / `.bar` recognition | `/app-api/app-repository/app-definitions` (list) ✅; `/app-api/app-runtime/app-instances` ❌ | 🟡 | **Partial.** App-definition browsing works. App-instances (runtime side) is not exposed in this image. For Flowatch's purpose — **recognizing `.bar` uploads and listing the bundled artifacts** — the repository side is enough. We don't need app-runtime to satisfy FR-55. |
