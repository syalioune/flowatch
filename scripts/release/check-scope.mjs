...
//
//   $ node scripts/release/check-scope.mjs reviews
//   ✓ scope `reviews` → 🎯 Review Platform
//
...
if (!rng || typeof rng[1]?.writerOpts?.transform !== 'function') {
  console.error(
    '✗ release.config.mjs is missing release-notes-generator or its transform.',
  );
  process.exit(2);
}
...
if (!themes) {
  console.error(
    '✗ Could not extract THEMES from release.config.mjs (regex parse failed).',
  );
  process.exit(2);
}
...
const QUALITY_GATES_CI_RE =
  /\b(mutation|stryker|pit|pitest|playwright|e2e|axe|coverage|jacoco)\b/i;
