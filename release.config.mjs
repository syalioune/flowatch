...
//      `!:` shortcut in subject lines (e.g. `feat(reviews)!: ...`)
...
//          guarantees between pre-1.0 releases — see
//          docs/plans/milestone-0.1.0-closure.md §1).
...
function detectMilestone() {
  ...
  // release/0.1.0 or release/0.1
  const releaseMatch = ref.match(/release\/(\d+)\.(\d+)(?:\.\d+)?/);
  if (releaseMatch) return `${releaseMatch[1]}.${releaseMatch[2]}.0`;
  // Fall back to the current local branch (handy for local previews).
  try {
    const branch = execSync('git rev-parse --abbrev-ref HEAD', {
      encoding: 'utf8',
    }).trim();
    const m = branch.match(/release\/(\d+)\.(\d+)(?:\.\d+)?/);
...
// Per-milestone editorial headline. Keyed by `<major>.<minor>` so
// patch releases inherit the minor's headline. Adding a new entry
// here is the only hand-edit needed per release — everything else
// derives from the conventional-commits range and the milestone
// state on GitHub.
const HEADLINES = {
  // Add an entry per minor release as it ships. Patch releases inherit
  // the minor's headline so '0.1.1' picks up '0.1.0'.
  // '0.0.1': 'Tech foundation — TS migration, Vitest + Playwright + Biome + TanStack Router, GitHub Actions CI, semantic-release wired.',
  // '0.1.0': '🎉 v1 MVP — parity rebuild. All 10 existing screens working against flowable-rest 7.2.0 on the bootstrapped stack.',
  // '0.2.0': '6.x parity gaps — process diagram overlay, variable edit, task edit + attachments, user/group lifecycle, batches, event subscriptions, app-definition browse, DMN execution history.',
  // '0.3.0': 'Operator polish — form-js designer, pluggable auth (Basic/Bearer/OIDC), engine version compatibility warning, Flowable-specific bpmn-js properties panel.',
  // '1.0.0': 'GA — accessibility audit complete, full Playwright snapshot coverage, compatibility-stable from this point.',
};

function headlineFor(version) {
  if (!version) return '';
  const key = version.split('.').slice(0, 2).join('.') + '.0';
...
function tallyLine(milestoneVersion, previousTag) {
  ...
  if (t.breaking != null) parts.push(`**${t.breaking} breaking** change${t.breaking === 1 ? '' : 's'}`);
...
const THEMES = [
  ...
  },
  {
    ...
    scopes: ['deployments', 'deployment', 'definitions', 'definition', 'repository'],
  },
  {
  ...
  },
  {
    ...
    scopes: ['identity', 'idm', 'users', 'user', 'groups', 'group', 'tenants', 'tenant', 'privileges'],
  },
  {
    name: '🔍 API Inspector',
  ...
  },
  {
    ...
    scopes: ['router', 'routing', 'routes', 'navigation', 'sidebar', 'topbar', 'chrome'],
  },
  {
  ...
  },
  {
    ...
    scopes: ['test', 'tests', 'vitest', 'playwright', 'e2e', 'coverage', 'lint', 'biome', 'typecheck'],
  ...
  {
    name: '📝 Documentation',
    scopes: ['docs', 'doc', 'readme', 'changelog', 'support', 'contributing', 'security', 'license'],
  },
  {
...
const customTransform = (commit, context) => {
  ...
  const mutableInput = reparseHeader({
    ...commit,
    notes: Array.isArray(commit.notes)
      ? commit.notes.map((n) => ({ ...n }))
      : commit.notes,
  ...
  const transformed = presetTransform
    ? presetTransform(mutableInput, context)
    : mutableInput;
  ...
  return {
    ...
    references: Array.isArray(transformed.references)
      ? transformed.references.filter(
          (r) => r && /^\d+$/.test(String(r.issue ?? '')),
        )
