...
**And** the DMN modeler chrome (toolbar, dropdown, decision-table editor handles) honors the active `data-density` consistently with the BPMN modeler (Stories 16.1–16.3), so toggling density in TweaksPanel rescales both modelers identically per UX Q-5.
