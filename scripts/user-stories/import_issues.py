...
class GitHubImporter:
    ...
    def create_issue(self, issue_data: IssueData, issue_number: int) -> bool:
        """Create a GitHub issue"""
        try:
            if self.dry_run:
                self.print_info(f"[{issue_number}] Would create issue: {issue_data.title}")
                if self.verbose:
            ...
            self.print_error(f"[{issue_number}] GitHub API error creating issue '{issue_data.title}': {e.data.get('message', str(e))}")
            return False
        except Exception as e:
            self.print_error(f"[{issue_number}] Unexpected error creating issue '{issue_data.title}': {str(e)}")
            return False
    ...
    def import_issues(self, csv_file_path: str) -> bool:
        ...
        if self.dry_run:
            self.print_warning("DRY RUN MODE - No issues will be created")
        
        for i, issue_data in enumerate(issues, start=1):
            if self.create_issue(issue_data, i):
    ...
    def print_summary(self):
        ...
        self.print_success(f"Successfully created: {self.stats['created_issues']}")
