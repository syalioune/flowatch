...
import { api, API_LOG } from './api.js';

export const Icon = ({ name, size = 14 }) => {
  const paths = {
    dashboard: <><rect x="3" y="3" width="7" height="9" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="5" rx="1.5"/><rect x="14" y="10" width="7" height="11" rx="1.5"/></>,
    bpmn: <><circle cx="5" cy="12" r="2.5"/><rect x="9" y="9" width="7" height="6" rx="1"/><circle cx="19" cy="12" r="2.5"/><path d="M7.5 12h1.5M16 12h1.5"/></>,
    dmn: <><rect x="3" y="4" width="18" height="16" rx="1.5"/><path d="M3 9h18M9 4v16"/></>,
    deploy: <><path d="M12 3l8 5v8l-8 5-8-5V8z"/><path d="M4 8l8 5 8-5M12 13v10"/></>,
    def: <><path d="M5 4h11l3 3v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1z"/><path d="M16 4v3h3M7 12h10M7 16h7"/></>,
    instance: <><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></>,
    job: <><path d="M3 7h18M3 12h18M3 17h12"/><circle cx="19" cy="17" r="2"/></>,
    task: <><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M7 9l2 2 5-5M7 15l2 2 5-5"/></>,
    history: <><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v5h5M12 7v5l3 2"/></>,
    identity: <><circle cx="12" cy="8" r="3.5"/><path d="M4 21c1.5-4 4.5-6 8-6s6.5 2 8 6"/></>,
    ...
    api: <><path d="M5 7h2l1 10h8l1-10h2M9 7V5a3 3 0 0 1 6 0v2"/></>,
    sun: <><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4 12H2M22 12h-2M5.5 5.5 4 4M20 20l-1.5-1.5M5.5 18.5 4 20M20 4l-1.5 1.5"/></>,
    moon: <path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/>,
    plus: <><path d="M12 5v14M5 12h14"/></>,
    play: <path d="M7 4l13 8-13 8z"/>,
    pause: <><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></>,
    save: <><path d="M19 21H5a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h11l4 4v13a1 1 0 0 1-1 1z"/><path d="M7 3v6h9V3M7 21v-7h10v7"/></>,
    ...
    tenant: <><path d="M3 21V8l9-5 9 5v13"/><path d="M9 21v-7h6v7"/></>,
    palette: <><circle cx="12" cy="12" r="9"/><circle cx="8.5" cy="10" r="1.5"/><circle cx="15.5" cy="10" r="1.5"/><circle cx="12" cy="15.5" r="1.5"/></>,
  };
  return (
...
export const EndpointChip = ({ method, path, onClick }) => (
  <button className="ep-chip" onClick={onClick} title={`${method} ${path}`}>
    <span className="ep-method" data-m={method}>{method}</span>
    <span>{path}</span>
  </button>
);
...
export const EndpointRow = ({ items, onOpenInspector }) => (
  <div className="ep-chip-row">
    {items.map((e, i) => (
      <EndpointChip key={i} method={e.method} path={e.path}
...
export const Topbar = ({ tenant, onTenant, theme, onTheme, onInspector, inspectorOpen, onSettings, onTweaks }) => (
  <div className="topbar">
    <div className="tenant-switch" onClick={onTenant}>
      <Icon name="tenant" size={13} />
      <span><b style={{fontWeight:500}}>{tenant.name}</b></span>
    ...
    <div className="top-actions">
      ...
      <button className="icon-btn" data-active={inspectorOpen ? "1" : "0"}
              title="API inspector" onClick={onInspector}>
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{width: 580}}>
        ...
        <div className="modal-bd">
          <div className="form-row">
            <label>Base URL <span className="mono">REST service root</span></label>
            <input className="input" value={cfg.baseUrl}
                   onChange={(e) => setCfg({...cfg, baseUrl: e.target.value})} />
            <div className="mute text-xs">
              Default OSS path: <span className="mono">http://localhost:8080/flowable-rest/service</span>
            </div>
          </div>
          ...
          <div className="form-row">
            <label>Tenant ID <span className="mono">optional</span></label>
            <input className="input" value={cfg.tenantId}
          ...
          <div style={{display:"flex", alignItems:"center", gap:10, marginTop:8}}>
            ...
            {pingRes && !pingRes.ok && (
              <span className="badge" data-tone="bad">{pingRes.error}</span>
            )}
          </div>
        </div>
        <div className="modal-ft">
          <button className="btn" onClick={onClose}>Cancel</button>
          <button className="btn" data-variant="primary" onClick={save}>Save</button>
        </div>
      </div>
    </div>
  );
...
const res = await fetch(
  "${u}",
  {
    ...
    headers: {
      ...
      "Accept":        "application/json"${ep.method !== "GET" ? `,
...
export const ApiInspector = ({ open, onClose, screenEndpoints, screenTitle }) => {
  ...
  const previewBody = (d) => {
    ...
    try { return JSON.stringify(d, null, 2).slice(0, 4000); }
  ...
  return (
    <div className="drawer" data-open={open ? "1" : "0"}>
      <div className="drawer-hd">
        ...
        <h3>API Inspector</h3>
        <button className="icon-btn x" onClick={onClose}><Icon name="x" size={14} /></button>
      </div>
      <div className="tabs" style={{margin:"0 18px"}}>
        <div className="tab" data-active={tab==="endpoints"?"1":"0"} onClick={() => setTab("endpoints")}>
          {screenTitle} endpoints
        </div>
        <div className="tab" data-active={tab==="calls"?"1":"0"} onClick={() => setTab("calls")}>
          Recent calls
          <span className="nav-count" style={{marginLeft:6}}>{log.length}</span>
        </div>
      </div>
      <div className="drawer-body">
              ...
              {(screenEndpoints || []).map((e, i) => (
                <div key={i} style={{padding:"10px 12px", border:"1px solid var(--line)",
                  ...
                  <div style={{display:"flex", alignItems:"center", gap:8}}>
                    <span className="ep-method" data-m={e.method}>{e.method}</span>
                    <span className="mono" style={{fontSize:12}}>{e.path}</span>
                  </div>
                  <div style={{fontSize:12, color:"var(--fg-soft)", marginTop:4}}>{e.desc}</div>
                </div>
              ))}
            </div>
            <div className="drawer-sect">Try it</div>
            <div className="seg-row" style={{marginBottom:8}}>
              <button className="seg-btn" data-on={snippet==="fetch"?"1":"0"} onClick={() => setSnippet("fetch")}>fetch()</button>
              <button className="seg-btn" data-on={snippet==="curl"?"1":"0"} onClick={() => setSnippet("curl")}>cURL</button>
              <button className="btn" data-size="sm" data-variant="primary"
                      disabled={running || tryPath.includes("{")}
                      onClick={runRequest}
                      style={{marginLeft:"auto"}}
                      title={tryPath.includes("{") ? "Replace {placeholders} first" : "Send a real request"}>
                <Icon name="play" size={11}/>
                {running ? "Running…" : "Run"}
              </button>
            </div>
            <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:8}}>
              <span className="ep-method" data-m={firstEp.method}>{firstEp.method}</span>
              <input className="input mono" style={{flex:1, fontSize:12}}
                     value={tryPath}
            ...
            {result && (
              <div className="try-result" data-ok={result.ok ? "1" : "0"}>
                <div className="try-result-hd">
                  <span className="status" data-s={result.ok ? "2xx" : (result.status >= 500 ? "5xx" : result.status >= 400 ? "4xx" : "err")}>
                    {result.status || "ERR"}
                  </span>
                  <span className="ms">{result.ms ?? 0}ms</span>
                  <button className="icon-btn x" style={{marginLeft:"auto"}}
                          onClick={() => setResult(null)}>
                    <Icon name="x" size={12}/>
                  </button>
                </div>
                <pre className="code" style={{marginTop:8, maxHeight:240, whiteSpace:"pre-wrap"}}>
        ...
        {tab === "calls" && (
          <>
            ...
            {log.length === 0 && (
              <div className="empty" style={{padding:20}}>No calls yet. Navigate around — every fetch shows up here.</div>
            )}
            {log.map((e) => (
...
export const PageHead = ({ title, subtitle, endpoints, actions, onOpenInspector }) => (
  <div className="page-head">
    <div>
      ...
      {endpoints && <div style={{marginTop:10}}>
