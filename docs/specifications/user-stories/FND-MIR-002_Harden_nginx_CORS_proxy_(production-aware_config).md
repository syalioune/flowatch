...
As Mira, I want the nginx CORS config to work in both local dev (allow `http://localhost:5173`) and any production origin the operator configures via `ALLOWED_ORIGIN`, so that the same Docker image works in both contexts.
...
**Then** running `docker compose up` with `ALLOWED_ORIGIN=https://your-flowatch-host.example` produces an nginx that allows that origin
