...
export type TweaksMessage =
  ...
  | { type: "__deck_rail_visible"; on: boolean }
  | { type: "__omelette_rail_enabled" };
...
export function TweaksPanel({
  ...
  const [railEnabled, setRailEnabled] = React.useState<boolean>(
    () =>
      hasDeckStage &&
      !!(document.querySelector("deck-stage") as (Element & { _railEnabled?: boolean }) | null)
        ?._railEnabled,
  );
  React.useEffect(() => {
  ...
  const toggleRail = (on: boolean) => {
    setRailVisible(on);
    window.postMessage({ type: "__deck_rail_visible", on } satisfies TweaksMessage, "*");
  };
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("message", onMsg);
    window.parent.postMessage({ type: "__edit_mode_available" } satisfies TweaksMessage, "*");
    return () => window.removeEventListener("message", onMsg);
  }, []);

  const dismiss = () => {
    setOpen(false);
    window.parent.postMessage({ type: "__edit_mode_dismissed" } satisfies TweaksMessage, "*");
  };
...
export type TweakOption<V extends string = string> = V | { value: V; label: string };

interface TweakRadioProps<V extends string> {
  label: string;
  value: V;
  options: Array<TweakOption<V>>;
  onChange: (v: V) => void;
}
  ...
  if (!fitsAsSegments) {
    ...
    return (
      <TweakSelect
        ...
        value={value}
        options={options}
        onChange={(s: V) => onChange(resolve(s))}
      />
    );
  }
