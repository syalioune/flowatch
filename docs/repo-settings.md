...
### CodeQL & Trivy
- **CodeQL** (SAST for JS/JSX) — wired in [`codeql.yml`](../.github/workflows/codeql.yml). Runs on push/PR to `main` + `develop`, on a weekly cron (Mon 06:17 UTC), and on `workflow_dispatch`. Language matrix is `javascript-typescript` (the combined pack — one entry covers `.js`, `.jsx`, and any future `.ts`/`.tsx`). Query suite is `security-extended` — richer than GitHub's default suite; Flowatch's source surface is small enough to triage the extra findings. Findings land in the Security tab → Code scanning alerts (category `/language:javascript-typescript`). The workflow is **not** in [`required_checks.json`](../.github/protection/required_checks.json) yet — there's a deliberate bake-in window before promoting it to a required check, so a noisy first run doesn't block unrelated PRs. Reproduce locally with `make codeql` (uses the [`gh codeql`](https://github.com/github/gh-codeql) CLI extension; writes `codeql.sarif` at the repo root — both the DB dir and the sarif file are gitignored).
