...
- **`scripts/user-stories/import_issues.py`** ingests that CSV once and creates the matching GitHub issues, applying labels, milestone, and persona from the file.
...
After import, GitHub issues are the source of truth for **state, labels, comments, PRs, ACs as completed**. Local files remain the source of truth for **the curated story body** (the hand-edited narrative that doesn't necessarily mirror the issue body verbatim).
...
| [`import_issues.py`](./import_issues.py) | One-shot import of a CSV-formatted backlog into GitHub issues. | Initial repo bootstrap only. |
| [`transform_user_stories.py`](./transform_user_stories.py) | Convert hand-written user-story markdown into the import-ready CSV. | Initial repo bootstrap only. |

The two Python scripts are **historical** — they were used to seed the issue tracker from a CSV at project start. The bash sync script is the **ongoing** mechanism.
